...
The current release produces SVG, PNG, JSON, and focused terminal views; `--deep` adds sampled blame-based ownership, survival, deleted-line interactions, and file coupling. `animate` adds a GIF recap and an optional Gource MP4, `explore` opens an interactive terminal explorer, and `external` optionally captures third-party analyses beside the report. See [architecture](docs/architecture.md) for the current data flow.
